...
<div class="manager-dashboard-update container">
    ...
    <div class="manager-dashboard-form">
        ...
        <?=$form->field($model, 'bool_fixed_or_individ')
            ->radioList([1 => 'Индивидуальная', 0 => 'Фиксированная'])
            ->label('Тип коммисии'); ?>
        ...
        $script = <<< JS
            ...
            if($("#managerdashboard-bool_fixed_or_individ input:checked").val() == 1){
                    $("#managerdashboard-tax").removeAttr("disabled");
            }
            else if ($("#managerdashboard-bool_fixed_or_individ input:checked").val() == 0) {
                $("#managerdashboard-tax").attr("disabled", "disabled");
            }

            var tax = $("#managerdashboard-tax").val();
            
            $('#managerdashboard-bool_fixed_or_individ input').change(function () {
                if($(this).val() == 1){
                    $("#managerdashboard-tax").removeAttr("disabled");
                }
                else if ($(this).val() == 0) {
                    $("#managerdashboard-tax").val(tax);
                    $("#managerdashboard-tax").attr("disabled", "disabled");
                }
...
JS;
        ...
        <?= $form->field($model, 'tax')->input('number', ['min' => 0])
            ->label('Коммисия')?>
        ...
        <?=$form->field($model, 'account_name')->textInput(['disabled' => true])->label('Профиль Instagram')?>
        <br>
